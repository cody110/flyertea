...
public class LoungeAdapter extends CommonAdapter<ArticleTabBean> {
    public LoungeAdapter(Context context, List<ArticleTabBean> datasAll, int layoutId) {
    ...
    public void convert(ViewHolder holder, ArticleTabBean articleTabBean) {
        ...
        if (!DataUtils.isEmpty(articleTabBean.getSortoptions())) {
            for (ArticleTabBean.SortoptionsBean bean : articleTabBean.getSortoptions()) {
                if (TextUtils.equals(bean.getIdentifier(), "lounge_airport")) {
