...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int indicatorColor;//  滑动条的颜色
    private int indicatorHeight = 2;// 滑动条的高度
    private int indicatorWidth = -1;//滑动条的宽度 默认-1

    private int underlineHeight = 0; // 滑动条所在的那个全宽线的高度
    ...
    private int tabTextSize = 18;// 文字的大小
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs,
        ...
        int textPrimaryColor = a.getColor(TEXT_COLOR_PRIMARY, getResources().getColor(R.color.white));
        ...
        indicatorHeight = a.getDimensionPixelSize(
                ...
                indicatorHeight);
        indicatorWidth = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsBottomLineWidth, indicatorWidth);
        ...
        isPaddingMiddle = a.getBoolean(
                ...
                isPaddingMiddle);
        tabTypefaceStyle = a
                .getInt(R.styleable.PagerSlidingTabStrip_pstsTextStyle,
                        Typeface.NORMAL);
        ...
        tabTextColor = a
                .getColorStateList(R.styleable.PagerSlidingTabStrip_pstsTextColorUnSelected);
...
//                : colorStateList;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        View currentTab = tabsContainer.getChildAt(currentPosition);
        View currentTv = currentTab.findViewById(R.id.psts_tab_title);

        //TODO 之前代码，是针对tab整块的底线，现在改为文字底线逻辑
        int tabWidth = currentTab.getWidth();
        int tvWidth = currentTv.getWidth();
        if (indicatorWidth != -1) {//TODO 没设置宽度，就为tv宽
            tvWidth = indicatorWidth;
        }
        int offset = (tabWidth - tvWidth) / 2;

        //底部
//        canvas.drawRect(lines.first + paddingLeft + offset, height - indicatorHeight,
//                lines.second + paddingLeft - offset, height, rectPaint);

        //上间距+tv高+line间距
        int lintTop = (height - currentTv.getHeight()) / 2 + currentTv.getHeight() + DensityUtil.dip2px(2);
        canvas.drawRect(lines.first + paddingLeft + offset, lintTop, lines.second + paddingLeft - offset, lintTop + indicatorHeight, rectPaint);
        // draw underline
