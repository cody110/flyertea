 ...
 * <pre>
 * LogFly.setEnabled(true);
 * LogFly.setEnabled(false);
 * </pre>
 * <p/>
 * 2.set the Tag for the log
 * LogFly.setTag("Android");
 * </pre>
 * <p/>
 * 3.log simple
 * <pre>
 ...
 * 5.log advance
 * <pre>
 * LogFly.d("test",new Throwable("test"));
 * LogFly.v("test",new Throwable("test"));
 * LogFly.i("test",new Throwable("test"));
 * LogFly.w("test",new Throwable("test"));
 * LogFly.e("test",new Throwable("test"));
 * </pre>
 * <p/>
 * 6.log advance -- set custom tag
 * <pre>
 * LogFly.d("TAG","test",new Throwable("test"));
 * LogFly.v("TAG","test",new Throwable("test"));
 * LogFly.i("TAG","test",new Throwable("test"));
 * LogFly.w("TAG","test",new Throwable("test"));
 * LogFly.e("TAG","test",new Throwable("test"));
 * </pre>
 * <p/>
 * 7.LogFly to File<BR>
 * log into one file
 * <pre>
 * LogFly.setPath("/mnt/sdcard/debug.txt");
 * LogFly.setPolicy(LogFly.LOG_ALL_TO_FILE);
 *
 * LogFly.d("test 1");
 * LogFly.v("test 2");
 * LogFly.i("test 3");
 ...
 * <pre>
 * LogFly.setPath("/mnt/sdcard/snowdream/log","log","log");
 * LogFly.setPolicy(LogFly.LOG_ALL_TO_FILE);
 *
 * LogFly.d("test 1");
 * LogFly.v("test 2");
...
//    protected static boolean isEnable = true;
     ...
     * The log dir path
     */
    protected static String logDirPath = "/mnt/sdcard/fly/android/log";
    /**
    ...
    public static void d(String tag, String msg) {
        if (FlyerApplication.LOGCAT_ENABLE) {
            if (tag == null || tag == "") {
                d(msg);
            } else {
    ...
    private static void createLogDir(String path) {
        ...
        if (!exist) {
            ...
            if (!ret) {
                android.util.Log.e("Error", "The LogFly Dir can not be created!");
                return;
            }
            android.util.Log.i("Success", "The LogFly Dir was successfully created! -" + file.getParent());
        }
    }
    ...
    public static boolean isEnabled() {
        return FlyerApplication.LOGCAT_ENABLE;
    }
    ...
    public static void setEnabled(boolean enabled) {
        FlyerApplication.LOGCAT_ENABLE = enabled;
    }
    ...
    public static void setPath(String logDirPath, String logFileBaseName, String logFileSuffix) {
        if (!TextUtils.isEmpty(logDirPath)) {
            LogFly.logDirPath = logDirPath;
        }

        if (!TextUtils.isEmpty(logFileBaseName)) {
            LogFly.logFileBaseName = logFileBaseName;
        }

        if (!TextUtils.isEmpty(logFileSuffix)) {
            LogFly.logFileSuffix = logFileSuffix;
        }
